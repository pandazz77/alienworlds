...
class client:
	...
	def checker(self):
		while True:
			self.s.send(self.key.encode())
			data = int((self.s.recv(1024)).decode())
...
cl = client(server_ip,server_port,config["key"])

while cl.get_access() == 0:
	print("Authorization...")
	time.sleep(1)
		...
		for cord in config["cords"]["farm_cords"]:
			...
			while farm_attempt<config["farm_attempt_limit"] and exec_stop is False:
				...
				try:
					...
					elif imgsearch.imagesearch("img/mine_in_menu.png")[0] != -1:
						pos = imgsearch.imagesearch("img/mine_in_menu.png")
						if pos[0]<0 or pos[1]<0:
							continue
						hc.move((pos[0]-config["oth_mons"]+config["calibrate"],pos[1]+config["calibrate"]), 1)
						hc.click()
